...
from app.schemas.order_schema import OrderDTO, ResponseOrderDTO, OrderResponseMessageDTO
...
@order_router.get("/orders/me", response_model=ResponseOrderDTO)
@login_required
async def get_orders_by_user(current_user: dict = Depends(get_current_user), db=Depends(get_db)):
    return get_user_orders(current_user, db)

@order_router.get("/orders", response_model=ResponseOrderDTO)
...
@order_router.delete("/orders/{order_id}", response_model=OrderResponseMessageDTO)
@login_required
...
@order_router.patch("/orders/{order_id}", response_model=OrderResponseMessageDTO)
@login_required
