...
def add_item_to_cart(user_id, product_id, quantity, db):
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="An error occurred while adding item to cart")
...
def remove_cart_item(cart_id, db):
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="An error occurred while removing cart item")